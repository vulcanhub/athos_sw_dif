...
#include "dif/regs/uart_regs.h"  // Generated.
...
/************************** BCI Modification **************************/
/* Added new Uart DIF functions.                                      */
/**********************************************************************/
dif_uart_result_t dif_uart_send_N_bytes(const dif_uart_t *uart,
                                        uint8_t *bytes,
                                        uint32_t count) {
  dif_uart_result_t status;

  for (uint32_t i = 0; i < count; ++i) {
    status = dif_uart_byte_send_polled(uart, bytes[i]);
    if (status != kDifUartOk) {
      return status;
    }
  }
...
dif_uart_result_t dif_uart_receive_N_bytes(const dif_uart_t *uart,
                                           uint8_t *bytes,
                                           uint32_t count) {
  dif_uart_result_t status;

  for (uint32_t i = 0; i < count; ++i) {
    status = dif_uart_byte_receive_polled(uart, bytes);
    if (status != kDifUartOk) {
      return status;
    }
